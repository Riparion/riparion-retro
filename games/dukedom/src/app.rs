...
fn GameRoot() -> Element {
        ...
        match snapshot.mode {
            ...
            Mode::GameOver => {
                ...
                if let Some(end) = snapshot.outcome.clone().filter(|e| !e.recorded) {
                    storage::record_score(HighScore::from_end(&snapshot.state.duke, &end));
                    drop(snapshot);
    ...
    let in_game = overlay_now == Overlay::Game
        && matches!(mode, Mode::Feed | Mode::Land | Mode::Plant | Mode::Interaction);
            ...
            div { class: "flex-1 overflow-y-auto flex flex-col",
                if overlay_now == Overlay::Help {
                    ...
                    match mode {
                        ...
                        Mode::Feed => rsx! { screens::feed::Feed {} },
                        Mode::Land => rsx! { screens::land::Land {} },
                        Mode::Plant => rsx! { screens::plant::Plant {} },

...
pub fn Fog() -> Element {
    ...
    let g = game.read();
    // A per-encounter seed (see `Game::encounter_seed`); the salt keeps a fog's
    // layout distinct from a same-fortnight flee or climb.
    let seed = g.encounter_seed(0xF06F_06F0_F06F_06F0);
    drop(g);

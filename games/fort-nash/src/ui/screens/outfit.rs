//! Spend the $700 grubstake across the five categories before taking the road.
...
const ROWS: [(&str, &str, f64); 5] = [
    ("Livestock", "$200–$300; a stronger train travels faster", 240.0),
    ("Provisions", "you'll eat ~13–23 lbs a week", 300.0),
    ("Powder & shot", "$1 buys a horn of 50 rounds", 60.0),
    ("Winter clothing", "blankets against the deep cold", 60.0),
    ("Supplies", "spare parts and medicine", 40.0),
];
    ...
    rsx! {
        div { class: "{SCREEN} gap-3",
            div { class: "text-center",
                h2 { class: "text-lg tracking-widest", "OUTFIT THE TRAIN" }
                p { class: "opacity-80 text-sm", "You have $700 in coin and trade goods." }
            }
            ...
            div { class: "flex justify-between text-lg px-1",
                span { class: if oxen_ok { "" } else { "chip-danger" },
                    if !oxen_ok { "Livestock must be $200–$300" } else { "Livestock ${oxen as i64}" }
                }

...
//! a botched one loses ground and supplies (and, for the snakebite, the medicine
...
//! from a same-fortnight splint, dose, steady trace, or brigade), and routes the
...
pub fn Sequence() -> Element {
    ...
    // distinct from a same-fortnight splint, dose, steady trace, or brigade.
    let seed = g.encounter_seed(0x5E9E_9CE0_5E9E_9CE0);
    drop(g);

    // Per-task framing, the step palette (words read as a procedure), how long the
    // sequence runs, and how fast it flashes. The deadliest one (snakebite) is the
    ...
    let (prompt, steps, length, show_ms): (&str, &[&str], usize, u32) = match task {
        SequenceTask::Wheel => (
            "A wagon wheel breaks down — re-hang it in order.",
            &["JACK", "BLOCK", "BOLT", "SEAT"],
            4,
            750,
        ),
        SequenceTask::OxLeg => (
            "An ox hurt its leg — dress it in sequence.",
            &["WRAP", "PAD", "BIND", "SPLINT"],
            4,
            700,
        ),
        SequenceTask::Snakebite => (
            "Bitten by a snake! Work the first-aid steps in order.",
            &["TIE", "LANCE", "DRAW", "WASH", "DRESS"],
            5,
            600,
        ),
    };

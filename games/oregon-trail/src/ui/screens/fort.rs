//! Shopping at a fort. Every dollar buys two-thirds its value in goods.
...
const ITEMS: [&str; 4] = ["Food", "Ammunition", "Clothing", "Misc. supplies"];
    ...
    rsx! {
        div { class: "{SCREEN} gap-3",
            div { class: "text-center",
                h2 { class: "text-lg tracking-widest", "TRADING POST" }
            ...
            div { class: "flex flex-col gap-2",
                for (i, label) in ITEMS.iter().enumerate() {
                    SpendRow {
                        ...
                        note: Some(if i == 1 {
                            format!("get {} bullets", (vals[1] as f64 * 2.0 / 3.0 * BULLETS_PER_DOLLAR) as i64)
                        } else {
            ...
            div { class: "text-center text-lg",
                span { class: if remaining < 0 { "chip-danger" } else { "" }, "Cash left: ${remaining}" }
            }

...
//! five supplies the original tracked across the bottom of the screen.
...
pub fn StatusBar() -> Element {
    ...
    rsx! {
        header { class: "status-bar shrink-0",
            ...
            div { class: "px-3 pb-1 text-xs opacity-70", "{group_thousands(remaining as i64)} miles to Oregon" }
            div { class: "grid grid-cols-5 gap-px text-center pb-1 px-1",
                Chip { label: "FOOD", value: fmt_num(s.food.max(0.0)), danger: low_food }
                Chip { label: "AMMO", value: fmt_num(s.bullets.max(0.0)), danger: low_ammo }
